...
//
// Resolved per call rather than at module load. Resolving once at import time
// bakes in whatever DSH_HOME happened to be during import, which is wrong for a
// long-lived process whose environment can change, and actively dangerous for
// tests: importing the module would capture the REAL home and a test's
// persist() would then write into the user's production ledger.
const storePaths = (env = process.env) => {
  const dir = join(resolveHarnessHome(env), "storages", "dsh-usage-bar");
  return { dir, file: join(dir, "usage.json") };
};
 ...
 */
function persist(store, log, paths) {
  try {
    mkdirSync(paths.dir, { recursive: true });
    writeAtomic(paths.file, serializeStore(store));
    return true;
  } catch (error) {
    persistFailures += 1;
    if (persistFailures === 1) {
      log?.warn?.(
        "dsh-usage-bar: could not persist the usage ledger to " + paths.file +
...
export function apply(ctx) {
  // Resolved once per apply() from the ambient environment at START time, not at
  // import time. A test that imports this module and then points DSH_HOME at a
  // temp directory therefore writes its temp store, never the user's real one.
  const paths = storePaths();
  const store = loadStore(paths.file);
  ...
  const scheduleSave = () => {
    ...
    saveTimer = setTimeout(() => {
      saveTimer = null;
      persist(store, ctx.logger, paths);
    }, 2000);
  };
  ...
  backfillTimer = setTimeout(() => {
    ...
    backfillOnce(store, persistedSessions(persistence, store))
      .then((added) => {
        if (added > 0) persist(store, ctx.logger, paths);
      })
      .catch((error) => {
